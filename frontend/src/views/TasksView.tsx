...
/* ---------- R17：任务检索（/api/runs/search，替换 slice(0,20) 硬截断） ---------- */
...
function RunSearchPanel({ onObserve }: { onObserve: (taskId: string) => void }) {
  ...
  return (
    <div className="rounded-xl border border-edge bg-surface p-3 shadow-card">
      <h2 className="mb-2 text-[13px] font-semibold">任务检索</h2>
      <input
        type="text"
        value={q}
...
/* ---------- R17 3.3：状态带（任务状态机显式化） ---------- */

/** 当前任务的相位 → 状态机节点。审批是唯一需要用户行动的中间态。 */
...
function StatusStrip({ task }: { task: TaskState }) {
  // 相位 → 当前节点索引：approval 挂起时高亮「待批准」
  const currentIdx =
    task.phase === "idle"
      ? -1
      : task.approval
...
const RUN_BADGE: Record<string, string> = {
  queued: "bg-surface-2 text-ink-3",
  ...
  interrupted: "bg-warn/10 text-warn",
};
...
function RunItem({
  ...
  // R17 状态收敛：未知/历史数据来源标记一律按 interrupted 风格兜底，可续跑
  const resumable = run.status !== "running" && run.status !== "queued";
  const title =
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className={`shrink-0 rounded-md px-1.5 py-0.5 text-[10.5px] font-medium ${RUN_BADGE[run.status] || RUN_BADGE.interrupted}`}>
...
export function TasksView() {
  ...
  const [leftView, setLeftView] = useState<"list" | "board">("list");
    ...
    <div className="flex h-full min-h-0">
      ...
      <div className="hidden w-64 shrink-0 flex-col gap-3 overflow-y-auto border-r border-edge p-3 lg:flex">
        ...
        <div className="min-h-0 flex-1">
          <RunSearchPanel onObserve={observeTask} />
          {/* R17 3.4：列表/看板切换（默认列表，看板为可选视图） */}
          <div className="mb-2 mt-3 flex items-center justify-between px-1">
            <h2 className="text-[13px] font-semibold">后台任务</h2>
            <div className="flex rounded-lg border border-edge text-[10.5px]">
              {(["list", "board"] as const).map((mode) => (
                <button
                  key={mode}
                  type="button"
                  onClick={() => setLeftView(mode)}
                  className={`px-2 py-0.5 transition-colors ${
                    leftView === mode
                      ? "bg-accent-tint font-medium text-accent-hover dark:text-accent"
          ...
          {leftView === "board" ? (
            <KanbanBoard tasks={durableTasks} onObserve={observeTask} />
          ) : (
          <>
