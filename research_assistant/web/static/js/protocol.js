 ...
 *
 * 与 docs/protocol.md 对齐；本文件不碰 DOM，供 node:test 覆盖。
 */
...
export function emptyTask() {
  return {
    phase: "idle",          // idle|running|done|failed|error|cancelled
    taskId: null,
    query: "",
    mode: null,             // pipeline | single | resume:<name>
    resumeRun: null,
    startedAt: null,
    finishedAt: null,
    timeline: TL_STAGES.map(() => "pending"),
    tlNote: {},             // key → 副文本（如 "3/5 节"）
    activity: [],           // [{t, kind, text, content?}]
    budget: null,
...
function addActivity(task, kind, text, content) {
  const activity = task.activity.concat({ t: Date.now(), kind, text, content });
...
function completeTimeline(tl) {
  return tl.map(() => "done");
}

/* 由消息文本推断更细的阶段（质量门/修订轮）；只前进不回退 */
function refineIdx(current, message) {
...
export function reduceTask(prev, msg) {
  const t = { ...prev, timeline: [...prev.timeline], tlNote: { ...prev.tlNote } };
  ...
  switch (msg.type) {
    ...
    case "progress": {
      ...
      if (msg.message) {
        const kind = /失败|错误|error/i.test(msg.message) ? "err"
          : /完成|成功|通过|✓/i.test(msg.message) ? "ok" : "log";
      ...
      if (stageChanged && stage) {
        const label = { initialization: "初始化", planning: "规划", research: "研究",
    ...
    case "result": {
      ...
      if (msg.status === "failed") {
        t.phase = "failed";
        t.error = (msg.errors || []).join("\n") || "文档生成失败";
        t.activity = addActivity(t, "err", t.error);
      } else {
        t.phase = "done";
        t.timeline = completeTimeline(t.timeline);
        t.activity = addActivity(t, "ok", `生成完成：${(msg.metadata || {}).title || msg.paper_name || ""}`);
      }
      t.approval = null;
      return t;
    }
...
/* 运行历史状态 → 徽标 class */
export function runBadgeClass(status) {
  return {
    running: "b-run", complete: "b-ok", failed: "b-err",
    cancelled: "b-warn", legacy: "",
  }[status] || "";
}

export const RUN_STATUS_LABEL = {
  running: "运行中", complete: "完成", failed: "失败",
  cancelled: "已停止", legacy: "早期文档",
};
