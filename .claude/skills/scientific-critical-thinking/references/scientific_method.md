...
The scientific method in engineering follows the same core principles but with domain-specific practices:

### Verification & Validation (V&V) — Engineering's Core Methodology

Engineering distinguishes between two fundamentally different questions:
- **Verification**: "Are we solving the equations correctly?" — Does the numerical solution converge to the exact solution of the mathematical model?
- **Validation**: "Are we solving the correct equations?" — Does the mathematical model adequately represent physical reality?

This V&V framework (per ASME V&V 10) is the engineering equivalent of the biomedical "does the treatment work?" question.

### Engineering Hypothesis Formation

| Domain | Example Hypothesis | Test Method |
|--------|-------------------|-------------|
| Structural | "Replaceable links achieve ductility ratio ≥ 3.0" | Quasi-static cyclic test per ATC-24 |
| Geotechnical | "Settlement is within 25mm for 50-year return period" | Consolidation test + FEM analysis |
| Environmental | "MBR achieves 95% COD removal at 6h HRT" | Pilot plant operation for 6 months |
| Computational | "GNN predicts damage location with >90% accuracy" | Cross-validation on experimental dataset |
| AI for Eng. | "PINNs satisfy equilibrium to within 5% residual" | Comparison with FEM reference solution |

### Engineering Evidence Standards

1. **Code-proven**: Design verified by decades of building code performance
2. **Experimentally validated**: Full-scale or reduced-scale test with proper instrumentation
3. **Numerically validated**: V&V per ASME V&V 10, mesh convergence, experimental comparison
4. **Numerically verified only**: Code verification without experimental validation (weaker)
5. **Analytical**: Closed-form solution with stated assumptions
6. **Empirical correlation**: Regression on limited data (weakest quantitative evidence)

### Engineering Reproducibility

Unlike biomedical studies, engineering experiments are often destructive and expensive (one-of-a-kind specimens). Reproducibility is achieved through:
- Detailed specimen fabrication specifications
- Standardized loading protocols (ASTM, GB, EN)
- Raw data sharing (load-displacement curves, strain data, crack patterns)
- Numerical model sharing (input files, mesh, boundary conditions)
- Independent verification by other researchers using different software/methods
