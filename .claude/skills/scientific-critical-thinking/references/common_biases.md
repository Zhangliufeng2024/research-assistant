...
### Engineering-Specific Biases

| Bias | Description | Example | Mitigation |
|------|-------------|---------|------------|
| **Confirmation bias in simulation** | Tuning model parameters until results match expected outcome | Adjusting material model until FEA matches test, ignoring physical basis | Blind validation: hide experimental results during model development |
| **Scale bias** | Assuming small-scale behavior applies directly at full scale | Using 1:10 scale RC test results for design without size effect correction | Apply Bažant size effect law; validate at multiple scales |
| **Mesh bias** | Accepting results without mesh convergence study | Reporting peak stress from single mesh without convergence check | Mandatory mesh convergence study; report convergence plot |
| **Boundary condition bias** | Over-simplifying supports/connections | Modeling pin supports when real connections have partial fixity | Sensitivity analysis on boundary conditions; match experimental setup |
| **Material model bias** | Using familiar constitutive model without justification | Using von Mises for concrete because it's the default in the software | Justify material model selection; compare alternatives |
| **Code compliance bias** | Assuming code minimum = adequate design | Designing for minimum load combinations without considering exceptional loads | Consider robustness, progressive collapse, exceptional scenarios |
| **Publication bias in engineering** | Negative results (failures) rarely published | Failed connections not reported, leading to biased design databases | Support failure case study publications; use forensic engineering data |
| **Automation bias** | Over-trusting software output without engineering judgment | Accepting FEA result with 500% stress concentration at point load without checking | Always sanity-check with hand calculations; compare with closed-form solutions |
| **Anchoring to design codes** | Treating code provisions as optimal rather than minimum consensus | Not improving beyond code minimum even when performance data supports it | Recognize codes as minimum; use performance-based design when beneficial |
| **Environmental LCA bias** | Selecting system boundary to favor desired conclusion | Excluding Scope 3 emissions to make product appear low-carbon | Follow ISO 14044 system boundary rules; conduct sensitivity analysis |
