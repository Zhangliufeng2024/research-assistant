...
| Level | Evidence Type | Example | Strength |
|-------|--------------|---------|----------|
| 1 | Code-proven designs with full-scale testing | ASCE 7 load combinations validated by decades of performance | Highest |
| 2 | Full-scale experimental validation | Shake table tests, full-scale structural tests, pilot plant trials | Very High |
| 3 | Reduced-scale experiments with similarity analysis | 1:4 scale RC columns, wind tunnel models | High |
| 4 | Validated numerical models (V&V per ASME V&V 10) | FEA calibrated against experiment, CFD validated by PIV | High |
| 5 | Verified numerical models (code verification only) | FEA with mesh convergence but no experimental comparison | Moderate |
| 6 | Analytical solutions with assumptions | Euler-Bernoulli beam theory, Darcy's law | Moderate |
| 7 | Expert judgment and engineering experience | Preliminary design estimates, code committee consensus | Low-Moderate |
| 8 | Unvalidated simulation or empirical correlation | CFD without validation, regression on limited data | Low |

## Engineering Appraisal Frameworks

### ASME V&V 10 — Verification & Validation
- **Verification**: "Are we solving the equations right?" — Code verification (Method of Manufactured Solutions), Solution verification (mesh convergence, time step convergence)
- **Validation**: "Are we solving the right equations?" — Comparison with experiment, quantification of validation uncertainty
- **Key metric**: Validation metric = |simulation - experiment| / √(u_sim² + u_exp²)

### Code Compliance Assessment
- Design must satisfy ALL applicable code clauses simultaneously
- Load combinations per ASCE 7 or GB 50009 — not arbitrary choices
- Material design values must come from code tables, not manufacturer claims
- Safety factors are MINIMUM values; higher factors for unusual conditions

### Scale and Similarity Assessment
- Froude similarity for gravity-dominated flows (hydraulic models)
- Reynolds number matching for viscous-dominated flows
- Cauchy similarity for aeroelastic models
- Scale effects on material strength (size effect law — Bažant)
- When similarity cannot be maintained, document which effects are lost

### Model Uncertainty Quantification
- Input uncertainty (material properties, loads, boundary conditions)
- Model form uncertainty (constitutive model selection, element type)
- Numerical uncertainty (discretization, convergence, solver tolerance)
- Propagation: Monte Carlo, Latin Hypercube, First-Order Second Moment (FOSM)
