...
## Engineering-Specific Logical Fallacies

### Appeal to Code Authority
**Fallacy**: "The design is safe because it satisfies the building code."
**Reality**: Building codes are minimum requirements, not optimal design targets. Code compliance does not guarantee adequate performance under exceptional loading (progressive collapse, disproportionate collapse, fire after earthquake).
**Detection**: Does the paper consider loading scenarios beyond code minimums?

### False Precision in Simulation
**Fallacy**: "The FEA peak stress is 347.82 MPa" — reporting 5 significant figures from a model with 15% input uncertainty.
**Reality**: Simulation precision is limited by input uncertainty, model form uncertainty, and numerical uncertainty combined.
**Detection**: Is the reported precision consistent with the input uncertainty?

### Composition Fallacy in Multi-Component Systems
**Fallacy**: "Each component passes its individual test, so the system is safe."
**Reality**: System-level behavior includes interaction effects, load redistribution, and progressive failure that component-level tests cannot capture.
**Detection**: Are system-level tests or analyses performed in addition to component tests?

### Hasty Generalization from Limited Scale Tests
**Fallacy**: "The 1:10 scale model survived, so the full-scale structure will too."
**Reality**: Scale effects (size effect in concrete, Reynolds number in fluid mechanics) can cause fundamentally different behavior at different scales.
**Detection**: Are scale effects explicitly addressed? Is the scale law validated?

### Cherry-Picking Mesh Results
**Fallacy**: "Our FEA shows the structure is safe" — using only one mesh without convergence study.
**Reality**: A single mesh result may be far from the converged solution, especially at stress concentrations.
**Detection**: Is a mesh convergence study reported?

### Confirmation Bias in Model Calibration
**Fallacy**: "We calibrated our model to match the experiment" — adjusting 5+ parameters until agreement is achieved.
**Reality**: With enough free parameters, any model can be fitted to any data (overfitting). Physical justification for each parameter is needed.
**Detection**: Are calibration parameters physically justified? Is the number of calibration parameters appropriate for the available data?

### Equating Correlation with Causation in Environmental Data
**Fallacy**: "Air pollution increased when the factory opened, so the factory caused the pollution."
**Reality**: Confounding factors (weather patterns, traffic changes, seasonal variation) must be controlled. Time series analysis with proper controls is needed.
**Detection**: Are confounding factors identified and controlled? Is the statistical approach appropriate for causal inference?

### Naturalistic Fallacy in Environmental Standards
**Fallacy**: "Natural treatment systems are better than engineered systems because they are natural."
**Reality**: "Natural" does not mean better, safer, or more effective. Performance depends on design, operation, and local conditions.
**Detection**: Is the comparison based on objective performance metrics or subjective preference?

### Appeal to Novelty in AI/ML Research
**Fallacy**: "Our deep learning model outperforms traditional methods" — without considering computational cost, interpretability, or physical consistency.
**Reality**: A model that is 2% more accurate but 1000x slower and physically inconsistent may not be "better" in practice.
**Detection**: Are practical deployment considerations (speed, cost, interpretability, physical consistency) evaluated?
