...
class RedliningValidator:
    ...
    def validate(self):
        ...
        # First, check if there are any tracked changes by Research-Assistant to validate
        try:
            ...
            # Check for w:del or w:ins tags authored by Research-Assistant
            del_elements = root.findall(".//w:del", self.namespaces)
            ins_elements = root.findall(".//w:ins", self.namespaces)

            # Filter to only include changes by Research-Assistant
            sw_del_elements = [
                elem
                for elem in del_elements
                if elem.get(f"{{{self.namespaces['w']}}}author") == "Research-Assistant"
            ]
            sw_ins_elements = [
                elem
            ...
            # Redlining validation is only needed if tracked changes by Research-Assistant have been used.
            if not sw_del_elements and not sw_ins_elements:
                if self.verbose:
                    print("PASSED - No tracked changes by Research-Assistant found.")
                return True
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Remove Research-Assistant's tracked changes from both documents
            ...
            if self.verbose:
                print("PASSED - All changes by Research-Assistant are properly tracked")
            return True

    def _generate_detailed_diff(self, original_text, modified_text):
        """Generate detailed word-level differences using git word diff."""
        error_parts = [
            "FAILED - Document text doesn't match after removing Research-Assistant's tracked changes",
            "",
            "Likely causes:",
    ...
    def _remove_sw_tracked_changes(self, root):
        """Remove tracked changes authored by Research-Assistant from the XML root."""
        ...
        for parent in root.iter():
            to_remove = []
            for child in parent:
                if child.tag == ins_tag and child.get(author_attr) == "Research-Assistant":
                    to_remove.append(child)
            for elem in to_remove:
                parent.remove(elem)

        # Unwrap content in w:del elements where author is "Research-Assistant"
        ...
        for parent in root.iter():
            to_process = []
            for child in parent:
                if child.tag == del_tag and child.get(author_attr) == "Research-Assistant":
