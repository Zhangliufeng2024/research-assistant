...
## ARGUMENTS (Route Immediately)

**When this skill is invoked with ARGUMENTS, parse and execute immediately. Do NOT output this document.**

| ARGUMENTS pattern | Action |
|---|---|
| `doi_to_bibtex <DOI>` | Run: `python .claude/skills/citation-management/scripts/doi_to_bibtex.py <DOI>` |
| `search_google_scholar <query>` | Run: `python .claude/skills/citation-management/scripts/search_google_scholar.py "<query>"` |
| `search_pubmed <query>` | Run: `python .claude/skills/citation-management/scripts/search_pubmed.py "<query>"` |
| `extract_metadata --doi <DOI>` | Run: `python .claude/skills/citation-management/scripts/extract_metadata.py --doi <DOI>` |
| `extract_metadata --pmid <PMID>` | Run: `python .claude/skills/citation-management/scripts/extract_metadata.py --pmid <PMID>` |
| `extract_metadata --arxiv <ID>` | Run: `python .claude/skills/citation-management/scripts/extract_metadata.py --arxiv <ID>` |
| `validate <file.bib>` | Run: `python .claude/skills/citation-management/scripts/validate_citations.py <file.bib>` |
| `format <file.bib>` | Run: `python .claude/skills/citation-management/scripts/format_bibtex.py <file.bib>` |
| `search <query>` | Run: `python .claude/skills/citation-management/scripts/search_google_scholar.py "<query>"` |

**IMPORTANT**: Always use the full path `.claude/skills/citation-management/scripts/` when running scripts. The `scripts/` relative path will NOT work.
...
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "CRISPR gene editing" \
  --limit 50 \
  --output results.json

# Search with year filter
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "machine learning protein folding" \
  --year-start 2020 \
  --year-end 2024 \
  --limit 100 \
...
```bash
# Search PubMed
python research_assistant/.claude/skills/citation-management/scripts/search_pubmed.py "Alzheimer's disease treatment" \
  --limit 100 \
  --output alzheimers.json

# Search with MeSH terms and filters
python research_assistant/.claude/skills/citation-management/scripts/search_pubmed.py \
...
```bash
# Convert single DOI
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1038/s41586-021-03819-2

# Convert multiple DOIs from a file
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py --input dois.txt --output references.bib

# Different output formats
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1038/nature12345 --format json
```
...
```bash
# Extract from DOI
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py --doi 10.1038/s41586-021-03819-2

# Extract from PMID
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py --pmid 34265844

# Extract from arXiv ID
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py --arxiv 2103.14030

# Extract from URL
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py --url "https://www.nature.com/articles/s41586-021-03819-2"

# Batch extraction from file (mixed identifiers)
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py --input identifiers.txt --output citations.bib
```
...
```bash
python research_assistant/.claude/skills/parallel-web/scripts/parallel_web.py search \
...
```bash
python research_assistant/.claude/skills/parallel-web/scripts/parallel_web.py extract \
...
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py references.bib \
...
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py references.bib
...
python research_assistant/.claude/skills/citation-management/scripts/extract_metadata.py \
  --input crispr_papers.json \
  --output crispr_refs.bib

# 3. Add specific papers by DOI
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1038/nature12345 >> crispr_refs.bib
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1126/science.abcd1234 >> crispr_refs.bib

# 4. Format and clean the BibTeX file
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py crispr_refs.bib \
  --deduplicate \
  --sort year \
  --descending \
...
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py my_review_references.bib --report review_validation.json

# Format for specific citation style if needed
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py my_review_references.bib \
  --style nature \
...
**Engineering Venue Quality Tiers:**
- **Tier 1 (Prefer):** Nature Energy, Nature Structural & Molecular Biology, PNAS (engineering topics)
- **Tier 2 (High Priority):** J. Structural Engineering (ASCE), Applied Energy, Energy & Environmental Science, Engineering Structures, Computer Methods in Applied Mechanics and Engineering (CMAME), Mechanical Systems and Signal Processing (MSSP)
- **Tier 3 (Good):** Energy and Buildings, Building and Environment, Construction and Building Materials, Automation in Construction, J. Bridge Engineering, Structural Health Monitoring
- **Tier 4 (Acceptable):** Advances in Structural Engineering, Journal of Computing in Civil Engineering, Structures, Thin-Walled Structures
- **Top Conferences:** ASCE Structures Congress, ICOSSAR, WCEE, IBPSA Building Simulation, IEEE PES General Meeting, SMiRT
...
Engineering research requires specialized databases beyond Google Scholar and PubMed.

**IEEE Xplore** (for IEEE/IET publications, standards, conference proceedings):
```bash
# Search IEEE Xplore via Google Scholar with source filter
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "finite element analysis seismic" \
  --source "IEEE" \
  --limit 50 \
  --output ieee_structural.json

# Or use direct Google Scholar operator
# site:ieeexplore.ieee.org "structural health monitoring" 2020..2024
```

**ASCE Library** (for ASCE journals: J. Structural Engineering, J. Bridge Engineering, etc.):
```bash
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "buckling-restrained brace seismic performance" \
  --source "Journal of Structural Engineering" \
  --limit 50 \
  --output asce_structural.json
```

**Engineering Village / Compendex** (broadest engineering coverage — access via institution):
- Primary database for ALL engineering disciplines
- Covers: civil, mechanical, electrical, chemical, environmental engineering
- Includes conference proceedings, journals, technical reports
- Use controlled vocabulary: "Finite element method" not just "FEM"
- Access: https://www.engineeringvillage.com/ (institutional subscription)

**Scopus** (multidisciplinary, strong in engineering):
```bash
python research_assistant/.claude/skills/parallel-web/scripts/parallel_web.py search \
  "scopus structural health monitoring machine learning 2020 2024" \
  -o sources/search_scopus_shm.md
```

**DOE OSTI** (US Department of Energy — energy, buildings, materials):
- Free, open access: https://www.osti.gov/
- Covers: building energy, renewable energy, materials science, grid technology
- Best for: EnergyPlus documentation, DOE building stock data, NREL reports

**NREL Technical Reports** (National Renewable Energy Laboratory):
- Free: https://www.nrel.gov/research/publications.html
- Covers: solar, wind, energy efficiency, buildings, grid integration

**TRID** (Transportation Research International Documentation):
- Free: https://trid.trb.org/
- Covers: transportation infrastructure, bridges, pavements, traffic engineering

**ISO / ASTM / ASCE Standards** (cite as @misc or @techreport):
```bibtex
@techreport{ASCE7-22,
...
**Engineering Citation Key Conventions**:
- Use: `AuthorYEARkeyword` format — e.g., `Smith2021seismic`, `Jones2022lca`
- For standards: `ASCE7-22`, `ACI318-19`, `ISO14040-2006`
- For reports: `NREL2023buildingstock`, `DOE2022comstock`
...
```bash
# Basic search
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "quantum computing"
...
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "machine learning" \
  --limit 50 \
  --format bibtex \
...
python research_assistant/.claude/skills/citation-management/scripts/search_pubmed.py "CRISPR gene editing"
...
# Export to BibTeX
python research_assistant/.claude/skills/citation-management/scripts/search_pubmed.py "Alzheimer's disease" \
  --limit 100 \
  --format bibtex \
...
# Multiple DOIs
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py \
  10.1038/nature12345 \
...
# Copy to clipboard
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1038/nature12345 --clipboard
```
...
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "transformer neural networks" \
  --year-start 2017 \
  --limit 50 \
  --output transformers_gs.json

python research_assistant/.claude/skills/citation-management/scripts/search_pubmed.py "deep learning medical imaging" \
  --date-start 2020 \
  --limit 50 \
...
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1038/s41586-021-03819-2 >> specific.bib
python research_assistant/.claude/skills/citation-management/scripts/doi_to_bibtex.py 10.1126/science.aam9317 >> specific.bib

# Step 4: Combine all BibTeX files
cat transformers.bib medical.bib specific.bib > combined.bib

# Step 5: Format and deduplicate
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py combined.bib \
  --deduplicate \
  --sort year \
  --descending \
...
# Step 6: Validate
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py formatted.bib \
  --auto-fix \
  --report validation.json \
  --output final_references.bib
...
# Validate the result
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py references.bib --verbose
```
...
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py messy_references.bib \
  --output step1_formatted.bib

# Step 2: Remove duplicates
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py step1_formatted.bib \
  --deduplicate \
  --output step2_deduplicated.bib

# Step 3: Validate and auto-fix
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py step2_deduplicated.bib \
  --auto-fix \
  --output step3_validated.bib

# Step 4: Sort by year
python research_assistant/.claude/skills/citation-management/scripts/format_bibtex.py step3_validated.bib \
  --sort year \
  --descending \
  --output clean_references.bib

# Step 5: Final validation report
python research_assistant/.claude/skills/citation-management/scripts/validate_citations.py clean_references.bib \
...
python research_assistant/.claude/skills/citation-management/scripts/search_google_scholar.py "AlphaFold protein structure" \
  --year-start 2020 \
  --year-end 2024 \
  --sort-by citations \
  --limit 20 \
...
- Engineering Village (Compendex): https://www.engineeringvillage.com/
- DOE OSTI: https://www.osti.gov/
- NREL Publications: https://www.nrel.gov/research/publications.html
- TRID Transportation: https://trid.trb.org/
