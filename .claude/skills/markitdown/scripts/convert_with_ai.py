...
This script demonstrates how to use MarkItDown with LLM API to generate
...
def convert_with_ai(
    ...
    try:
        ...
        print(f"闁?Successfully converted to: {output_file}")
        return True
        
    except Exception as e:
        print(f"闁?Error: {str(e)}", file=sys.stderr)
        return False
...
Environment Variables:
  LLM_API_KEY    API key (required if not passed via --api-key)
...
Popular Models (use with --model):
  ...
  google/gemini-pro-vision   - Gemini Pro Vision
    ...
    parser.add_argument(
        ...
        help='Model to use via LLM API (default: anthropic/claude-sonnet-4.5)'
    )
    parser.add_argument(
        '--prompt-type', '-t',
    ...
    if not api_key:
        print("Error: API key required. Set LLM_API_KEY environment variable or use --api-key")
