...
|-------|----------|----------|----------------|
| `anthropic/claude-sonnet-4.5` | Anthropic | **Recommended** - Best overall for scientific analysis | 闁?|
| `anthropic/claude-opus-4.5` | Anthropic | Excellent technical analysis | 闁?|
| `openai/gpt-4o` | OpenAI | Strong vision understanding | 闁?|
| `openai/gpt-4-vision` | OpenAI | GPT-4 with vision | 闁?|
| `google/gemini-pro-vision` | Google | Cost-effective option | 闁?|
...
# Initialize LLM API client (OpenAI-compatible)
client = OpenAI(
...
All skill scripts have been updated to use LLM API:
...
LLM API pricing varies by model. Check current rates at https://openrouter.ai/models
...
3. Consider upgrading your LLM API plan

## Migration Notes

This skill was updated from direct OpenAI API to LLM API. Key changes:

1. **Environment Variable**: `OPENAI_API_KEY` 闁?`LLM_API_KEY`
2. **Client Initialization**: Added `base_url="https://openrouter.ai/api/v1"`
3. **Model Names**: `gpt-4o` 闁?`openai/gpt-4o` (with provider prefix)
4. **Script Updates**: All scripts now use LLM API by default
...
Here's a complete workflow using LLM API:
