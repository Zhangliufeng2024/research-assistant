# LLM API integration for MarkItDown

## Overview

This MarkItDown skill has been configured to use **LLM API** instead of direct OpenAI API access. LLM API provides a unified API gateway to access 100+ AI models from different providers through a single, OpenAI-compatible interface.
...
|-------|----------|----------|----------------|
| `anthropic/claude-sonnet-4.5` | Anthropic | **Recommended** - Best overall for scientific analysis | 閴?|
| `anthropic/claude-opus-4.5` | Anthropic | Excellent technical analysis | 閴?|
| `openai/gpt-4o` | OpenAI | Strong vision understanding | 閴?|
| `openai/gpt-4-vision` | OpenAI | GPT-4 with vision | 閴?|
| `google/gemini-pro-vision` | Google | Cost-effective option | 閴?|
...
# Make it permanent
echo 'export LLM_API_KEY="sk-or-v1-..."' >> ~/.zshrc  # macOS
echo 'export LLM_API_KEY="sk-or-v1-..."' >> ~/.bashrc # Linux
...
# Initialize OpenAI-compatible client (OpenAI-compatible)
client = OpenAI(
    api_key="your-llm-api-key",  # or use env var
...
All skill scripts have been updated to use an OpenAI-compatible API:
...
API provider pricing varies by model. Check current rates at https://openrouter.ai/models
...
LLM API has rate limits. If you hit them:
1. Add delays between requests
2. Use batch processing scripts with `--workers` parameter
3. Consider upgrading your API provider plan

## Migration Notes

This skill was updated from direct OpenAI API to LLM API. Key changes:

1. **Environment Variable**: `OPENAI_API_KEY` 閳?`LLM_API_KEY`
2. **Client Initialization**: Added `base_url="https://openrouter.ai/api/v1"`
3. **Model Names**: `gpt-4o` 閳?`openai/gpt-4o` (with provider prefix)
4. **Script Updates**: All scripts now use an OpenAI-compatible API by default

## Resources

- **LLM API Website**: https://openrouter.ai
...
Here's a complete workflow using an OpenAI-compatible API:
