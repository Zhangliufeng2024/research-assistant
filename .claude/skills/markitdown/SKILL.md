...
Use LLMs via LLM API to generate detailed image descriptions (for PPTX and image files):
...
# Use LLM API for access to multiple AI models
client = OpenAI(
    api_key="your-llm-api-key",
    base_url="https://openrouter.ai/api/v1"
)
...
for file in files:
    try:
        ...
        with open(output, "w") as f:
            f.write(result.text_content)
        print(f"闁?Converted {file}")
    except Exception as e:
        print(f"闁?Error converting {file}: {e}")
```
...
- **LLM API**: https://openrouter.ai (for AI-enhanced conversions)
