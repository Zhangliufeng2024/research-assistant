...
All diagrams MUST follow the **Unified Visual Style Guide** at `figure-generation/references/unified_visual_style.md`. Before generating any diagram, read that file and include its "AI-Generated Diagrams" directives in your prompt to ensure visual consistency with all other figures in the project.
...
3. **Decision**: If quality >= threshold 闁?**DONE** (no more iterations needed!)
...
- 闁?Saves API calls if first generation is good enough
- 闁?Higher quality standards for journal papers
- 闁?Faster turnaround for presentations/posters
- 闁?Appropriate quality for each use case
...
闁?**Good prompts** (specific, detailed):
- "CONSORT flowchart showing participant flow from screening (n=500) through randomization to final analysis"
- "Transformer neural network architecture with encoder stack on left, decoder stack on right, showing multi-head attention and cross-attention connections"
- "Biological signaling cascade: EGFR receptor 闁?RAS 闁?RAF 闁?MEK 闁?ERK 闁?nucleus, with phosphorylation steps labeled"
- "Block diagram of IoT system: sensors 闁?microcontroller 闁?WiFi module 闁?cloud server 闁?mobile app"

闁?**Avoid vague prompts**:
...
- Structural analysis diagrams (beams, frames, moment/shear diagrams)
- FEA mesh visualizations with stress contours
- Cross-section details (RC beams, steel sections, foundations)
- Load path diagrams showing force flow through structures
- Response spectra and seismic design diagrams
- Water/wastewater treatment process flows
- LCA system boundary and carbon lifecycle diagrams
- Contaminant plumes and remediation systems
- Digital twin and structural health monitoring architectures
- Smart building IoT system diagrams
- Hybrid FEA-ML workflow diagrams
...
- 闁?Layout and composition
- 闁?Labels and annotations
- 闁?Colors and styling
- 闁?Quality review and refinement
- 闁?Publication-ready output
...
```
闁崇懓澶囬弨銏ゅ煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘?
闁? 1. Generate image with Nano Banana Pro             闁?
闁?                   闁?                               闁?
闁? 2. Review quality with Gemini 3 Pro                闁?
闁?                   闁?                               闁?
闁? 3. Score >= threshold?                             闁?
闁?      YES 闁?DONE! (early stop)                      闁?
闁?      NO  闁?Improve prompt, go to step 1            闁?
闁?                   闁?                               闁?
闁? 4. Repeat until quality met OR max iterations      闁?
闁宠鏌￠弨銏ゅ煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘閳ь剟鍩為埀顒勫煘?
```
...
**Example:**
- For a **poster** (threshold 7.0): Score of 7.5 闁?**DONE after 1 iteration!**
- For a **journal** (threshold 8.5): Score of 7.5 闁?Continue improving
...
```
闁?"Flowchart with vertical flow, top to bottom"
闁?"Architecture diagram with encoder on left, decoder on right"
闁?"Circular pathway diagram with clockwise flow"
```

**2. Include Quantitative Details:**
```
闁?"Neural network with input layer (784 nodes), hidden layer (128 nodes), output (10 nodes)"
闁?"Flowchart showing n=500 screened, n=150 excluded, n=350 randomized"
闁?"Circuit with 1k閹?resistor, 10绾板瓗 capacitor, 5V source"
```

**3. Specify Visual Style:**
```
闁?"Minimalist block diagram with clean lines"
闁?"Detailed biological pathway with protein structures"
闁?"Technical schematic with engineering notation"
```

**4. Request Specific Labels:**
```
闁?"Label all arrows with activation/inhibition"
闁?"Include layer dimensions in each box"
闁?"Show time progression with timestamps"
```

**5. Mention Color Requirements:**
```
闁?"Use colorblind-friendly colors"
闁?"Grayscale-compatible design"
闁?"Color-code by function: blue for input, green for processing, red for output"
```
...
### Example 5: Structural Analysis Diagram (Civil Engineering)
```bash
python scripts/generate_schematic.py \
  "Structural analysis diagram of a simply supported reinforced concrete beam. \
   Span 6m, distributed load 10 kN/m, point load 50 kN at midspan. \
   Show the beam with supports (pin left, roller right) at top. \
   Below: Bending moment diagram (parabolic, max 112.5 kN鐠虹棳 at center, labeled). \
   Below that: Shear force diagram (step function, max 55 kN at supports). \
   Label reactions: R1=55 kN (left), R2=55 kN (right). \
   Use blue for beam, red for sagging moment, green for hogging." \
  -o figures/beam_analysis.png --doc-type journal
```

### Example 6: Water Treatment Process Flow (Environmental Engineering)
```bash
python scripts/generate_schematic.py \
  "Water treatment plant process flow diagram. \
   Horizontal layout left-to-right: \
   Raw water intake (river icon) 闁?Coagulation tank (alum dosing, 30 mg/L) 闁?\
   Flocculation (3 chambers) 闁?Sedimentation basin (HRT 3h) 闁?\
   Rapid sand filtration 闁?Activated carbon adsorption 闁?\
   Chlorine disinfection (contact 30 min) 闁?Clear water storage 闁?Distribution. \
   Show sludge line below: Sedimentation sludge 闁?Thickener 闁?Dewatering 闁?Disposal. \
   Label removal efficiencies: turbidity 99%, BOD 95%, bacteria 99.9%. \
   Use blue for water, brown for sludge, green for treatment units." \
  -o figures/water_treatment.png --doc-type journal
```

### Example 7: LCA System Boundary (Environmental / Sustainability)
```bash
python scripts/generate_schematic.py \
  "Life cycle assessment system boundary diagram for reinforced concrete building. \
   Cradle-to-grave showing 5 stages in a horizontal flow: \
   Stage 1: Raw material extraction (quarry, cement plant) - gray. \
   Stage 2: Manufacturing (ready-mix plant) - blue. \
   Stage 3: Construction (site work) - orange. \
   Stage 4: Use phase (50 years, maintenance, energy) - green. \
   Stage 5: End-of-life (demolition, recycling 60%, landfill 40%) - brown. \
   Show system boundary box enclosing all stages. \
   Include transportation arrows between stages. \
   Mark cut-off criteria: 1% mass, 1% energy." \
  -o figures/lca_boundary.png --doc-type journal
```

### Example 8: Digital Twin Architecture (AI + Engineering)
```bash
python scripts/generate_schematic.py \
  "Digital twin architecture for bridge structural health monitoring. \
   Left: Physical bridge with sensors (strain gauges, accelerometers, GPS, temperature). \
   Middle: Data pipeline - DAQ system 闁?Edge computing (feature extraction) 闁?Cloud platform. \
   Right: Digital model - Physics-based FEA model + ML surrogate model running in parallel. \
   Bottom: Feedback loop - Predictions 闁?Decision support 闁?Maintenance scheduling 闁?Physical bridge. \
   Show real-time data streaming with arrow labeled '5G/LoRa'. \
   Use color coding: blue for physical, green for digital, orange for data flow." \
  -o figures/digital_twin.png --doc-type journal
```
