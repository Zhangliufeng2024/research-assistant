...
```bash
python scripts/generate_schematic.py \
  "MAPK signaling pathway: EGFR receptor 闁?RAS 闁?RAF 闁?MEK 闁?ERK 闁?nucleus. \
...
```bash
python scripts/generate_schematic.py \
  "IoT system block diagram: sensors (bottom) 闁?microcontroller 闁?\
   WiFi module and display (middle) 闁?cloud server 闁?mobile app (top). \
...
闁?"Make a diagram" (too vague)

### Include Quantitative Details
闁?"Neural network: input (784), hidden (128), output (10)"  
闁?"Flowchart: n=500 screened, n=150 excluded, n=350 randomized"  
闁?"Some numbers" (not specific)
...
闁?"Color-code by function: blue=input, green=processing, red=output"
...
- 闁?**Fast**: Results in minutes
- 闁?**Easy**: Natural language descriptions (no coding)
- 闁?**Quality**: Automatic review and refinement
- 闁?**Universal**: Works for all diagram types
- 闁?**Publication-ready**: High-quality output immediately
...
echo 'export LLM_API_KEY="your_key"' >> ~/.bashrc
```
...
LLM API pricing for models used:
...
Part of the research-assistant package. See main repository for license information.
