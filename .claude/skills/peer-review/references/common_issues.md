...
### Chinese Standards Issues
42. **Outdated Standard Edition**: Paper references obsolete GB/JGJ standard (e.g., using GB 50010-2002 instead of 2010 revision). Check: always cite the latest edition with revision year.
43. **Inconsistent Load Combinations**: Dead/live/wind load partial safety factors don't match GB 50009. Check: γ_G=1.3 (or 1.0 for favorable), γ_Q=1.5, γ_W=0.6 for ULS.
44. **Material Property Mismatch**: Claimed concrete/rebar grades don't match GB standard characteristic values. Check: C30 → f_ck=20.1 MPa, HRB400 → f_yk=400 MPa.
45. **Missing Standard Test Citation**: Experimental study doesn't cite the GB test method standard. Check: Every test procedure should cite GB/T 50081, GB/T 228.1, etc.
46. **Mixed Standards Without Reconciliation**: Paper mixes GB and ACI/Eurocode without explaining equivalences. Check: If both are used, provide conversion/comparison table.

### Environmental Experiment Issues
47. **Insufficient Adsorbent Characterization**: Adsorption study lacks BET surface area, pore analysis, or morphological characterization. Fix: Require BET, SEM, FTIR at minimum.
48. **Missing Isotherm Model Comparison**: Only one isotherm model fitted without comparison. Fix: Fit at least Langmuir and Freundlich, report R² and χ² for comparison.
49. **No Regeneration Study**: Adsorption study reports capacity but not reusability. Fix: Require at least 3 adsorption-desorption cycles.
50. **Inadequate QA/QC for Water Analysis**: Missing blanks, duplicates, or spike recovery data. Fix: Require MDL/MQL, recovery 80-120%, RPD <20% for duplicates.
51. **Membrane Fouling Not Characterized**: Flux decline reported without fouling mechanism analysis. Fix: Require reversible/irreversible fouling decomposition, fouling mechanism identification.
52. **Biological Treatment Influent Variability**: Influent characterization insufficient or inconsistent. Fix: Require steady-state verification with <10% variation in key parameters.
53. **Atmospheric Model Not Validated**: Dispersion model results without comparison to monitoring data. Fix: Require validation metrics (FB, NMSE, r, FAC2).
54. **Discharge Standards Not Cited**: Treatment efficiency reported without comparison to regulatory limits. Fix: Always compare effluent with applicable standard (GB 18918, EU Directive, EPA).
