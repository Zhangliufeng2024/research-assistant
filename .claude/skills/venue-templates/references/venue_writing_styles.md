...
## Engineering Journals (ASCE, Elsevier, IEEE)

### Core Philosophy

Engineering journals prioritize **technical rigor**, **practical applicability**, and **reproducibility**. The question is not "Why should a general scientist care?" but "Does this advance engineering knowledge in a verifiable and applicable way?"

### ASCE Journals Writing Style

**Covers**: J. Structural Engineering, J. Bridge Engineering, J. Engineering Mechanics, J. Geotechnical Engineering, J. Computing in Civil Engineering, Natural Hazards Review

**Key Writing Principles**:
1. **Quantify everything**: Replace "significant improvement" with "23% increase in lateral load capacity"
2. **State design code context**: Always cite the specific edition (ASCE 7-22, ACI 318-19, AISC 341-22)
3. **Numbered conclusions**: ASCE strongly prefers a numbered list of ≤6 concise conclusions
4. **Data Availability Statement**: Required since 2022 — state where data/code can be accessed
5. **SI units mandatory**: Include US customary in parentheses only if needed for practice context
6. **Author-year citations**: `(Smith and Jones 2021)` not numbered brackets

**Abstract Style (ASCE)**:
- ≤250 words, unstructured
- Must include: gap statement, objectives, methods (with software/database named), quantitative results, conclusions
- State specific numbers: "RMSE = 0.045 mm", "38–62% amplification", "IDR exceeded 2.5% in 35% of records"

**Example — ASCE abstract opening**:
> "Buckling-restrained braces (BRBs) have been widely adopted in steel moment frames, yet their performance under near-fault pulse-type ground motions remains poorly characterized. This study investigates the seismic response of nine-story BRB-SMRF prototypes subjected to 40 pulse-type records from the PEER NGA-West2 database using OpenSees v3.5, validated against published experimental data (prediction error ≤4.3%)."

**Structure**:
Introduction → Background/Theory (optional) → Methodology → Results → Discussion → Conclusions (numbered) → Data Availability → Acknowledgments → References

---

### Elsevier Engineering Journals Writing Style

**Covers**: Engineering Structures, Applied Energy, Energy and Buildings, Building and Environment, Construction and Building Materials, J. Cleaner Production, CMAME, MSSP, Renewable Energy, Structural Safety

**Key Writing Principles**:
1. **Contribution bullets**: Many Elsevier engineering editors now expect 3–5 bullet-point highlights at the end of the abstract or in the Highlights section
2. **Graphical abstract**: Required by most Elsevier engineering journals — one image (400–1333 px wide) summarizing the key result
3. **CRediT statement**: Required since 2020 — assign specific roles to each author
4. **Active voice preferred**: "We developed a model" not "A model was developed"
5. **Numbered citations**: `[1]`, `[2,3]` in order of appearance; never author-year
6. **Declaration of competing interest**: Mandatory section

**Abstract Style (Elsevier Engineering)**:
- 200–250 words, unstructured
- Sequence: gap → objective → method (software + database named) → quantitative result → conclusion
- Keywords: 4–8 terms, do not repeat title words

**Highlights format** (required by Applied Energy, Energy and Buildings, etc.):
```
• Coupled LCA–BES framework applied to 120 UK dwellings across 5 archetypes
• Heat pump + insulation retrofit achieves 58 ± 6% whole-life carbon reduction
• Embodied carbon contributes 8–14% of 50-year whole-life carbon budget
• LCCS of £47–63/tCO₂e vs. £110–180/tCO₂e for PV-only pathway
• Envelope-first strategy recommended for temperate climate decarbonization
```

**Structure**:
Introduction → Methodology → Results → Discussion → Conclusions (numbered) → CRediT Statement → Declaration of Competing Interest → Data Availability → Acknowledgements → References

---

### IEEE Transactions Writing Style (Engineering)

**Covers**: IEEE Trans. on Power Systems, IEEE Trans. on Smart Grid, IEEE Trans. on Sustainable Energy, IEEE Trans. on Industrial Electronics, IEEE Access

**Key Writing Principles**:
1. **Formal, technical tone**: Passive voice common and acceptable ("The algorithm is implemented in MATLAB")
2. **Roman numeral sections**: `\section{I. Introduction}` — IEEE convention
3. **Problem formulation section**: Engineering IEEE papers typically have an explicit mathematical problem formulation section
4. **Two-column final format**: Use `IEEEtran` class; `\documentclass[journal]{IEEEtran}`
5. **Index Terms**: 3–5 IEEE taxonomy terms after abstract
6. **Numbered citations**: `[1]`, `[2]`; max 6 authors before "et al."

**Abstract Style (IEEE)**:
- ≤200 words, unstructured
- Sequence: problem → proposed method → key result (with numbers) → conclusion
- Dense and technical — not written for non-specialists

**Example — IEEE abstract opening**:
> "This paper proposes a physics-informed neural network (PINN) framework for real-time structural damage detection in reinforced concrete frames under seismic excitation. The governing equation of motion is embedded as a soft constraint in the loss function, enabling training on sparse sensor data (n=8 accelerometers). On the PEER SMRF benchmark dataset, the proposed method achieves 94.3% damage localization accuracy with a computational latency of 12 ms per frame, compared to 87.1% for a standard CNN baseline."

---

## Engineering Writing Style Comparison

| Aspect | ASCE | Elsevier Engineering | IEEE Engineering |
|--------|------|---------------------|-----------------|
| Citation style | Author-year (Smith 2021) | Numbered [1] | Numbered [1] |
| Voice | Active/passive mixed | Active preferred | Passive common |
| Abstract | ≤250 words, unstructured | 200–250 words + Highlights | ≤200 words |
| Conclusion style | Numbered list | Numbered list | Prose or numbered |
| Graphical abstract | Not standard | Required (most journals) | Not required |
...
| Design codes | ASCE/ACI/AISC/Eurocode | ISO/ASHRAE/IEC | IEEE Std / IEC |
| LaTeX class | `article` + `natbib` | `elsarticle` | `IEEEtran` |

---

## Adapting Between Engineering and Other Venue Types

### Engineering Journal → Nature/Science

1. **Lead with societal impact**: "This threatens $X trillion in infrastructure" not "This affects D/C ratios"
2. **Remove design code references from main text**: Move to methods/supplementary
3. **Replace jargon with plain language**: "building energy consumption" not "EUI in kBtu/ft²/yr"
4. **Add conceptual figures**: Schematic of the problem, not just results plots
5. **Broaden discussion**: Policy implications, climate change context, global relevance

### Engineering Journal → ML Conference (NeurIPS/ICML)

1. **Add contribution list**: Explicit numbered bullets in introduction
2. **Strengthen baselines**: Add standard ML baselines (MLP, GBM, LSTM) not just domain methods
3. **Add ablation studies**: Show effect of each model component
4. **Reproducibility section**: Seeds, hyperparameters, compute requirements, code release
5. **Reframe engineering context**: "Domain-specific challenge" framing, not assuming civil engineering knowledge

### Quick Checklist — Engineering Submissions
- [ ] Design code edition cited (ASCE 7-22, not just "ASCE 7")
- [ ] Software and version stated (OpenSees 3.5.0, EnergyPlus v23.1, ABAQUS 2024)
- [ ] Validation/calibration metric reported (RMSE, CVRMSE, MBE, R²)
- [ ] SI units used consistently with siunitx
- [ ] Quantitative conclusions (not "the method performs well")
...
- `engineering_journal_styles.md` - Full ASCE / Elsevier / IEEE engineering style guide
