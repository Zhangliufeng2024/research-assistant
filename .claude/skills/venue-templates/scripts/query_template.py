...
# Ensure UTF-8 output on Windows (prevents GBK codec crash on Unicode print chars)
...
TEMPLATES = {
    "journals": {
        ...
        "asce": {
            "file": "asce_journal.tex",
            "full_name": "ASCE Journal",
            "description": "American Society of Civil Engineers journal (structural, geotechnical, environmental engineering)",
            "page_limit": "~8000 words",
            "citation_style": "Author-year (ASCE)",
            "format": "Single column"
        },
        "elsevier_engineering": {
            "file": "elsevier_engineering.tex",
            "full_name": "Elsevier Engineering Journals",
            "description": "Elsevier engineering journals (Engineering Structures, Computer Methods in Applied Mechanics, etc.)",
            "page_limit": "~8000 words",
            "citation_style": "Numbered [1] (Elsevier Harvard)",
            "format": "Single column"
        },
        "ieee": {
            "file": "ieee_trans.tex",
            "full_name": "IEEE Transactions",
            "description": "IEEE Transactions journals (TPAMI, TNNLS, TSE, etc.) and IEEE conference papers",
            "page_limit": "8-12 pages (conference), ~14 pages (journal)",
            "citation_style": "Numbered [1]",
            "format": "Two column"
        }
    },
...
def search_templates(venue=None, template_type=None, keyword=None):
    ...
    for cat_name, category in TEMPLATES.items():
        ...
        for temp_id, template in category.items():
            # Filter by venue name (bidirectional keyword matching)
            if venue:
                venue_lower = venue.lower()
                name_lower = template.get("full_name", "").lower()
                # Check if query is a substring of template name, OR if any significant word from query appears in template id/name
                if (venue_lower not in temp_id and
                    venue_lower not in name_lower and
                    not any(word in temp_id or word in name_lower
                            for word in venue_lower.split() if len(word) > 2)):
                    continue
