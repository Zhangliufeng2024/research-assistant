...
description: Access comprehensive LaTeX templates, formatting requirements, and submission guidelines for major scientific publication venues. General science: Nature, Science, PLOS, Cell. Engineering & Built Environment: ASCE journals (JSE, JEM, JBE), Structural Safety, Energy & Buildings, Applied Energy, Building and Environment, Computers & Structures. CS/AI conferences: NeurIPS, ICML, CVPR, CHI, ICLR, ACL. Interdisciplinary: IEEE, ACM, Elsevier engineering journals. Grant proposals: NSF (CMMI/CBET/EFRI/ECCS), NIH, DOE BTO/EERE/ARPA-E, FHWA, DARPA. Use when preparing manuscripts for journal submission, conference papers, research posters, or grant proposals and need venue-specific formatting requirements.
...
|---|---|
| `get <venue> template` | Run: `python .claude/skills/venue-templates/scripts/query_template.py --venue "<venue>"` |
| `list templates` | Run: `python .claude/skills/venue-templates/scripts/query_template.py --list` |
| `search <keyword>` | Run: `python .claude/skills/venue-templates/scripts/query_template.py --keyword "<keyword>"` |
| `customize <template> title=<T> authors=<A>` | Run: `python .claude/skills/venue-templates/scripts/customize_template.py` with appropriate args |
| `validate <pdf>` | Run: `python .claude/skills/venue-templates/scripts/validate_format.py <pdf>` |

**IMPORTANT**: Always use the full path `.claude/skills/venue-templates/scripts/` when running scripts. The `scripts/` relative path will NOT work.
...
**Civil & Structural Engineering Journals**:
...
**Energy & Environmental Engineering Journals**:
- Nature Energy (Nature Portfolio)
- Energy & Environmental Science (RSC) — highest impact energy journal
...
**AI & Computational Engineering Journals**:
...
- ASCE Structures Congress (annual, US-focused)
...
- International Conference on Computational Methods in Structural Dynamics and Earthquake Engineering (COMPDYN)

**Energy & Environmental Engineering**:
- IEEE PVSC (Photovoltaic Specialists Conference)
- IEEE PES General Meeting (Power & Energy Society)
- ASHRAE Annual Conference and Building Performance Analysis Conference
- Sustainable Built Environment Conference (SBE)
- eceee Summer Study on Energy Efficiency in Buildings
- International Building Performance Simulation Association (IBPSA) Building Simulation
...
# Retrieve template
Template: assets/journals/nature_article.tex          # Nature / biomedical
Template: assets/journals/asce_journal.tex            # ASCE engineering journals
Template: assets/journals/elsevier_engineering.tex    # Elsevier engineering journals

# Engineering style guide
Reference: references/engineering_journal_styles.md   # ASCE / Elsevier / IEEE engineering style
```
...
```bash
python research_assistant/.claude/skills/venue-templates/scripts/customize_template.py \
...
```bash
python research_assistant/.claude/skills/venue-templates/scripts/validate_format.py \
  --file my_paper.pdf \
  --venue "Nature" \
  --check-all
```
...
| **Civil & Structural Engineering** | ASCE journals, Engineering Structures, Structural Safety, Computers & Structures |
| **Energy & Environmental Engineering** | Applied Energy, Energy & Environmental Science, Renewable Energy, J. Cleaner Production, Building and Environment |
| **AI for Engineering** | Computer Methods in Applied Mechanics, Eng. Applications of AI, Mechanical Systems and Signal Processing |
| **Computer Science** | ACM, IEEE, NeurIPS, ICML, ICLR |
| **Medicine** | NEJM, Lancet, JAMA, BMJ |
| **Interdisciplinary** | PNAS, Nature Communications, Science Advances, Nature Energy |
...
python research_assistant/.claude/skills/venue-templates/scripts/query_template.py --venue "Nature" --type "article"

# Search by keyword
python research_assistant/.claude/skills/venue-templates/scripts/query_template.py --keyword "machine learning"

# List all available templates
python research_assistant/.claude/skills/venue-templates/scripts/query_template.py --list-all

# Get requirements for a venue
python research_assistant/.claude/skills/venue-templates/scripts/query_template.py --venue "NeurIPS" --requirements
```
...
# Interactive mode
python research_assistant/.claude/skills/venue-templates/scripts/customize_template.py --interactive
```
...
- `asce_structural_abstract_example.md`: ASCE structural/civil engineering abstract + introduction
- `energy_journal_abstract_example.md`: Applied Energy / Elsevier energy journal abstract + highlights + introduction

**Templates** (in `assets/`):
- `journals/nature_article.tex`: Nature family journals
- `journals/neurips_article.tex`: NeurIPS / ML conference papers
- `journals/plos_one.tex`: PLOS ONE and open-access biomedical journals
- `journals/asce_journal.tex`: ASCE journals (J. Structural Eng., J. Bridge Eng., J. Computing in Civil Eng., etc.) — author-year citations
- `journals/elsevier_engineering.tex`: Elsevier engineering journals (Engineering Structures, Applied Energy, Energy and Buildings, J. Cleaner Production, Construction and Building Materials, etc.) — numbered citations, elsarticle class
- `posters/`: Research poster templates
- `grants/`: Grant proposal templates

**Engineering Style Reference** (in `references/`):
- `engineering_journal_styles.md`: Complete writing style guide for ASCE, Elsevier Engineering, and IEEE engineering journals — abstract format, structure, citation style, figure requirements, and journal-specific notes
