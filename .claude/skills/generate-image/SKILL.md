---
name: generate-image
description: Generate or edit images using AI models (gpt imagine, FLUX, Gemini). Use for general-purpose image generation including photos, illustrations, artwork, visual assets, concept art, and any image that isn't a technical diagram or schematic. For flowcharts, circuits, pathways, and technical diagrams, use the scientific-schematics skill instead.
allowed-tools: [Read, Write, Edit, Bash]
---

# Generate Image

Generate and edit high-quality images using LLM API's image generation models including FLUX.2 Pro and Gemini 3 Pro.
...
All images for scientific documents MUST follow the **Unified Visual Style Guide** at `figure-generation/references/unified_visual_style.md`. Before generating any image, read that file and include its "AI-Generated Diagrams" directives in your prompt to ensure visual consistency with all other figures in the project.
...
**CRITICAL**: The script requires an LLM API key. Before running, check if the user has configured their API key:

1. Look for a `.env` file in the project directory or parent directories
2. Check for `LLM_API_KEY=<key>` in the `.env` file
3. If not found, inform the user they need to:
   - Create a `.env` file with `LLM_API_KEY=your-api-key-here`
   - Or set the environment variable: `export LLM_API_KEY=your-api-key-here`
...
- `--model` or `-m`: LLM model ID (default: google/gemini-3-pro-image-preview)
- `--output` or `-o`: Output file path (default: generated_image.png)
- `--api-key`: LLM API key (overrides .env file)
...
- The script supports both `images` and `content` response formats from different LLM models
...
- Check LLM API pricing for cost information: https://openrouter.ai/models
...
- Both Gemini 3 Pro and FLUX.2 Pro support image editing through LLM API
