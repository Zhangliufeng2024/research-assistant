...
## Engineering Experimental Design Patterns

### Pattern: FEM Validation Experiment

**Purpose:** Validate a finite element model against physical experiment

**Design:**
- Specimen design follows code provisions (GB 50010, GB 50011, ACI 318, AISC 360)
- Loading protocol: monotonic, cyclic (ATC-24), or dynamic (shake table)
- Instrumentation: strain gauges, LVDTs, accelerometers, DIC (digital image correlation)
- Measurement points: at least 3 replicates per critical location

**Variables:**
- Independent: geometry, material properties, loading protocol, boundary conditions
- Dependent: load-displacement response, strain distribution, failure mode, crack pattern
- Controlled: specimen fabrication QC, curing conditions, test setup alignment

**Analysis:**
- Mesh convergence study (≤5% change in key response metric)
- Sensitivity analysis on uncertain parameters (material properties, boundary conditions)
- Validation metric: |simulation - experiment| / √(u_sim² + u_exp²) < 1.0

**Sample size:** Minimum 3 identical specimens for statistical reliability

---

### Pattern: Structural Component Testing

**Purpose:** Determine load-carrying capacity, ductility, and failure mode of structural component

**Design:**
- Quasi-static loading per applicable standard (ASTM E8, GB/T 228.1, GB/T 50152)
- Displacement-controlled loading for post-peak behavior
- Monotonic or cyclic protocol per research objectives

**Variables:**
- Independent: cross-section dimensions, reinforcement ratio, material grade, connection type
- Dependent: ultimate load, yield displacement, ductility ratio, energy dissipation, failure mode
- Controlled: steel mill certificate, concrete batch, curing regime, test setup rigidity

**Quality metrics:**
- Ductility ratio μ = Δ_u / Δ_y (≥3.0 for ductile behavior)
- Energy dissipation per cycle (hysteretic loops)
- Strength degradation ratio per cycle
- Stiffness degradation ratio per cycle

**Sample size:** Minimum 3 per parameter set; 5+ for statistical analysis

---

### Pattern: Material Characterization Testing

**Purpose:** Obtain constitutive model parameters for numerical analysis

**Design:**
- Standard test methods: ASTM E8 (steel tension), ASTM C39 (concrete compression), GB/T 228.1 (metallic materials), GB/T 50081 (concrete)
- Multiple strain rates if rate-dependent behavior expected
- Temperature effects if thermal loading relevant

**Variables:**
- Independent: material grade, strain rate, temperature, specimen geometry
- Dependent: yield strength, ultimate strength, elastic modulus, elongation, stress-strain curve
- Controlled: specimen machining tolerances, alignment, loading rate

**Analysis:**
- True stress-strain conversion from engineering values
- Constitutive model fitting (Ramberg-Osgood, concrete damage plasticity)
- Statistical characterization: mean, COV, characteristic value (5th percentile)

**Sample size:** Minimum 5 per material grade per condition; 10+ for probabilistic analysis

---

### Pattern: Field Monitoring Campaign

**Purpose:** Collect long-term performance data from real structures or systems

**Design:**
- Sensor placement per structural analysis (critical sections, load paths)
- Sampling frequency: Nyquist criterion for dynamic events; 1 Hz minimum for quasi-static
- Duration: minimum 1 year for environmental effects; 10+ years for deterioration

**Variables:**
- Independent: environmental conditions (temperature, humidity, wind), traffic/loading, age
- Dependent: strain, displacement, acceleration, crack width, corrosion rate
- Controlled: sensor calibration schedule, data acquisition system reliability

**Analysis:**
- Time series analysis: trend, seasonality, anomaly detection
- Extreme value statistics for peak responses
- Correlation analysis between environmental and structural response
- Deterioration modeling: Markov chains, gamma processes

**Quality control:**
- Redundant sensors at critical locations (≥2 per measurement point)
...
### Pattern: Prototype System Testing

**Purpose:** Validate complete system performance before deployment

**Design:**
- Test in operational or near-operational conditions
- Performance metrics per design specifications
- Failure mode identification through progressive loading/testing

**Variables:**
- Independent: system configuration, operating parameters, environmental conditions
- Dependent: performance metrics (efficiency, throughput, accuracy, durability)
- Controlled: assembly procedures, operator training, test environment

**Analysis:**
- Comparison with design targets
- Identification of performance envelope
- Failure mode and effects analysis (FMEA)
- Reliability analysis: Weibull, exponential

**Sample size:** Minimum 3 prototypes; more for high-reliability applications

---

### Pattern: Scale Model Testing

**Purpose:** Study behavior at reduced scale when full-scale testing is impractical

**Design:**
- Similarity requirements: geometric, kinematic, dynamic (Froude, Reynolds, Cauchy)
- Scale factor selection based on dominant physics
- Model material selection to maintain similarity

**Variables:**
- Independent: scale factor, model material, loading rate
- Dependent: scaled response (displacement, force, frequency, pressure)
- Controlled: model fabrication precision, boundary condition fidelity

**Analysis:**
- Scale law derivation and verification
- Scale effect quantification (e.g., Bažant size effect for concrete)
- Comparison with full-scale data when available
- Limitation documentation (which physics cannot be scaled)

**Sample size:** Minimum 2 scale ratios to verify scale law; 3+ replicates per scale
