...
- **Perplexity sonar-pro-search** (via LLM API): Used only for academic-specific paper searches where scholarly database access is critical.
...
- Model: `perplexity/sonar-pro-search` (via LLM API)
...
```bash
# Auto-routed research (recommended) 閳?ALWAYS save to sources/
python research_lookup.py "your query" -o sources/research_YYYYMMDD_HHMMSS_<topic>.md

# Force specific backend 閳?ALWAYS save to sources/
...
# JSON output 閳?ALWAYS save to sources/
python research_lookup.py "your query" --json -o sources/research_<topic>.json

# Batch queries 閳?ALWAYS save to sources/
...
```bash
# General research 閳?save to sources/ (includes Sources + Additional References sections)
...
# Academic paper search 閳?save to sources/ (includes paper citations with DOIs)
...
# Forced backend 閳?save to sources/
...
# Batch queries 閳?save to sources/
...
|--------|-------------------|-------------|
| Text (default) | `Sources (N):` section with `[title] (date) + URL` + `Additional References (N):` with DOIs and academic URLs | Standard use 閳?human-readable with all citations |
...
1. **Literature Review Support**: Gather current research for introduction and discussion 閳?**save to `sources/`**
2. **Methods Validation**: Verify protocols against current standards 閳?**save to `sources/`**
3. **Results Contextualization**: Compare findings with recent similar studies 閳?**save to `sources/`**
4. **Discussion Enhancement**: Support arguments with latest evidence 閳?**save to `sources/`**
5. **Citation Management**: Provide properly formatted citations 閳?**save to `sources/`**
