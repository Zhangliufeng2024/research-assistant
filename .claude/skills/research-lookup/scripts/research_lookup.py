...
Routes research queries to the best backend:
  ...
  - Perplexity sonar-pro-search (via LLM API): Academic-specific paper searches

Environment variables:
  PARALLEL_API_KEY    - Required for Parallel Chat API (primary backend)
  LLM_API_KEY         - Required for Perplexity academic searches (fallback)
...
def _find_project_root():
    """Walk up from this script to find model_config.py in the project root."""
    current = Path(__file__).resolve().parent
    for _ in range(8):
        if (current / "model_config.py").exists():
            return str(current)
...
class ResearchLookup:
    ...
    def _perplexity_lookup(self, query: str) -> Dict[str, Any]:
        """Run academic search via Perplexity sonar-pro-search through LLM API."""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        api_key = (model_config.get_api_key() if model_config else None) or os.getenv("LLM_API_KEY")
        model = model_config.get_search_model() if model_config else "perplexity/sonar-pro-search"
        ...
        try:
            response = requests.post(
                (model_config.get_base_url() if model_config else "https://openrouter.ai/api/v1") + "/chat/completions",
                headers=headers,
                json=data,
                timeout=90,
            )
...
Examples:
  ...
  python research_lookup.py "topic" --json -o results.json
    ...
    has_perplexity = bool(os.getenv("LLM_API_KEY"))
    if not has_parallel and not has_perplexity:
        ...
        print("  export LLM_API_KEY='...'         (fallback - Perplexity academic)", file=sys.stderr)
        if output_file:
            output_file.close()
        return 1
