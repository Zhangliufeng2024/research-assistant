...
PALETTE_PRESETS = {
    "wong": {
        "name": "Okabe-Ito (Publication Standard)",
        "colors": "orange (#E69F00), sky blue (#56B4E9), bluish green (#009E73), blue (#0072B2), vermillion (#D55E00), reddish purple (#CC79A7), yellow (#F0E444), black (#000000)",
    },
    "ibm": {
...
- White space: adequate margins, not cramped"""
    ...
    INFOGRAPHIC_GUIDELINES = _UNIFIED_STYLE + """
    ...
    def __init__(self, api_key: Optional[str] = None, verbose: bool = False):
        ...
        if not self.api_key:
            raise ValueError(
                ...
                "Set IMAGE_API_KEY in your .env file"
            )
...
Include citation hints where possible."""
        ...
        try:
            ...
            research_model = os.getenv("SEARCH_MODEL", "perplexity/sonar-pro")
...
Be concise and factual. Focus on information useful for an infographic."""
        ...
        try:
            ...
            search_model = os.getenv("SEARCH_MODEL", "perplexity/sonar-pro")
...
Incorporate specific numbers, percentages, and dates from the research."""
    ...
    def _make_request(self, model: str, messages: List[Dict[str, Any]],
                     modalities: Optional[List[str]] = None, max_retries: int = 3) -> Dict[str, Any]:
        """Make a request to LLM API with retry logic."""
        import random
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self._log(f"Rate limited (429). Retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                    _time.sleep(delay)
                    last_exception = RuntimeError(f"API request failed (HTTP 429): rate limited")
                elif response.status_code >= 500:
                    delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                    self._log(f"Server error ({response.status_code}). Retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                    _time.sleep(delay)
                    last_exception = RuntimeError(f"API request failed (HTTP {response.status_code}): server error")
                else:
                    error_detail = response_json.get("error", response_json)
                    raise RuntimeError(f"API request failed (HTTP {response.status_code}): {error_detail}")
            except requests.exceptions.Timeout:
                delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                self._log(f"Request timed out. Retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                _time.sleep(delay)
                last_exception = RuntimeError("API request timed out after 120 seconds")
            except requests.exceptions.RequestException as e:
                delay = base_delay * (2 ** attempt) + random.uniform(0, 1)
                self._log(f"Request failed: {e}. Retrying in {delay:.1f}s (attempt {attempt + 1}/{max_retries})")
                _time.sleep(delay)
    ...
    def generate_iterative(self, user_prompt: str, output_path: str,
        ...
        if research:
            ...
            if research_result.get("success"):
                print(f"鉁?Research complete - gathered facts and statistics")
                ...
                print(f"鉁?Research saved: {research_path}")
            else:
                print(f"鈿?Research failed: {research_result.get('error', 'Unknown error')}")
        ...
        for i in range(1, iterations + 1):
            ...
            if not needs_improvement:
                print(f"\n鉁?Quality meets threshold ({score} >= {threshold})")
...
Environment:
  ...
  IMAGE_REVIEW_MODEL    Quality review model (default: agnes-2.0-flash)
    ...
    try:
        ...
        if results["success"]:
            print(f"\n鉁?Success! Infographic saved to: {args.output}")
